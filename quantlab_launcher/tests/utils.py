...
from quantlab_launcher import QuantLabLauncherApp, QuantLabConfig
...
class LabTestBase(NotebookTestBase):
    ...
    def setup_class(cls):
        ...
        lab_config = QuantLabConfig(schemas_dir=pjoin(here, 'schemas'),
            user_settings_dir=tmp('user_settings'))

        def start_thread():
            app = cls.notebook = QuantLabLauncherApp(
                port=cls.port,
                port_retries=0,
                open_browser=False,
